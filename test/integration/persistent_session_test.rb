...
class PersistentSessionTest < ActionDispatch::IntegrationTest
  test "session persists via signed cookie when session is empty" do
    ...
    post create_host_sessions_path, params: { name: "Tobi" }
    ...
    assert cookies.signed[:user_id].present?, "Signed user_id cookie should be present"
    assert_equal user.id, cookies.signed[:user_id]
  end

  test "logout clears both session and cookie" do
    post create_host_sessions_path, params: { name: "Tobi" }
    assert cookies.signed[:user_id].present?
